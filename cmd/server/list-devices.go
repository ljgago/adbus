...
// ListDevices list of all devices connected to broker
func (srv *deviceService) ListDevices(ctx context.Context, req *types.Empty) (*pb.ListDevicesResponse, error) {
	var (
		wg   sync.WaitGroup
		m    sync.Mutex
	...
	sub, err := srv.conn.Subscribe("server.list.v1.devices", func(msg *nats.Msg) {
		wg.Add(1)
		go listDevices(&resp, msg, &wg, &m)
	})

	if err != nil {
		log.Error().Str("type", "server").Err(err).Msg("")
		return &pb.ListDevicesResponse{}, err
	}
...
func listDevices(resp *pb.ListDevicesResponse, msg *nats.Msg, wg *sync.WaitGroup, m *sync.Mutex) {
	data := &pb.Device{}
